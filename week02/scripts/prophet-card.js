...
// 填充克隆的模板
// 使用 for 迴圈遍歷所有的先知
for (let i = 0; i < prophet.prophets.length; i++) {
    const prophetData = prophet.prophets[i];  // 取得當前先知資料

    // 克隆模板內容
    const cardClone = document.importNode(template.content, true);

    // 填充克隆的模板
    cardClone.querySelector('h2').textContent = `${prophetData.name} ${prophetData.lastname}`;
    cardClone.querySelectorAll('p')[0].textContent = prophetData.birthdate;
    cardClone.querySelectorAll('p')[1].textContent = prophetData.birthplace;
    cardClone.querySelector('img').src = prophetData.imageurl;
    cardClone.querySelector('img').alt = `Profile of ${prophetData.name} ${prophetData.lastname}`;
